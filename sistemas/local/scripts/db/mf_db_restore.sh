...
echo "######################################################################################################"
echo "Restaurar Backup de Base de Datos - Material Farmacias"
echo "######################################################################################################"
echo
...
case $ENV_OPTION in
  1) PATTERN="bd__*__*.sql.gz" ;;
  2) PATTERN="bd__local__*.sql.gz" ;;
...
echo
echo "ADVERTENCIA: Esto SOBRESCRIBIRÁ la base de datos actual: ${MF_DB_NAME} en ${MF_ENV}"
...
TEMP_FILE=$(mktemp /tmp/mf_restore_XXXXXX.sql)
...
mysql --protocol=TCP -h ${MF_DB_HOST} -P ${MF_DB_PORT} -u ${MF_DB_USER} -p ${MF_DB_NAME} < ${TEMP_FILE}
